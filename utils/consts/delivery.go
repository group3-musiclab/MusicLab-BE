package consts

// Error Response
const (
	HANDLER_ErrorIdParam           string = "id param must number"
	HANDLER_ErrorFormFile          string = "error form file"
	HANDLER_ErrorBlobFile          string = "blob file not found"
	HANDLER_InvalidPageParam       string = "invalid page parameter"
	HANDLER_InvalidLimitParam      string = "invalid limit parameter"
	HANDLER_InvalidIdStudent       string = "id student must number"
	HANDLER_InvalidIdMentor        string = "id mentor must number"
	HANDLER_InvalidRatingParam     string = "rating query param must number"
	HANDLER_InvalidInstrumentParam string = "instrument query param must number"
	HANDLER_InvalidGenreParam      string = "genre query param must number"
)
